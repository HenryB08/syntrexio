...
- **Two legacy pages held, no clean equivalent (per "tell me before redirecting"):** `legal.html` (combined legal; React splits into /privacy and /terms) and `syn.html` (old product page; the product no longer exists). They are NOT redirected yet and will keep serving legacy content after cutover until a target is chosen. Recommended: `legal.html` -> `/terms/`, `syn.html` -> `/services/` (or `/`).
